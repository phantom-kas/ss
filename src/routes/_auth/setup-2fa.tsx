...
const handleVerify = async (code: string) => {
  ...
  try {
    ...
    authActions.updateUser({ totp_enabled: true }); // 👈 this is all you need
    setStep('success');
  } catch (e) {
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-6 space-y-4">
      ...
      {/* Verify — uses your reusable TotpPage */}
      {step === 'verify' && (
        <TotpPage
          title="Enter the 6-digit code"
          subtitle="From your authenticator app to confirm setup"
          submitLabel="Confirm & Enable 2FA"
          submitLoadingLabel="Verifying..."
          isLoading={isVerifying}
          error={totpError}
          onSubmit={handleVerify}
          onCancel={() => { setStep('scan'); setTotpError(''); }}
          cancelLabel="← Back to QR code"
        />
      )}
      ...
      {step === 'success' && (
        <Card>
          ...
          <CardFooter>
            <Button variant="blue" className="w-full" onClick={() => navigate({ to: '/settings/security' })}>
      ...
      {step === 'error' && (
        <Card>
          ...
          <CardFooter>
            <Button variant="outline" className="w-full" onClick={init}>Try Again</Button>
          </CardFooter>
        </Card>
      )}
    </div>
